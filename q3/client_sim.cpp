...
void *client_func(void *idx)
{
    ...
    s[index] = to_string(index) + " " + s[index];
    send_string_on_socket(socket_fd, s[index]);
    int num_bytes_read;
    string output_msg;
...
void begin_process()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        *idx = i;
        pthread_create(&clients[i], NULL, client_func, (void *)idx);
    }
    /*
    while (true)
    {
